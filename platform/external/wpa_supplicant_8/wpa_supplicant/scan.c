...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (ssid && max_ssids == 1) {
		 ...
		 */
#ifdef CONFIG_MTK_HIDDEN_SSID
		/*
		* don't schedule a wildcard scan here, because MTK's
		* driver always do it if supplicant trigger
		* any scan to driver.
		* if we don't remove this code, time in UI scan list of
		* APs with hidden ssid will be reduced
		*/
		wpa_s->prev_scan_ssid = ssid;
		wpa_s->prev_scan_wildcard = 0;
