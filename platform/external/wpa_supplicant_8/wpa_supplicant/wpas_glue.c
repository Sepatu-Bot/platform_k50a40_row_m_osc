...
static int wpa_supplicant_tx_wai(void *ctx, const u8* pbuf, int length)
{
	...
	int ret;
	ret = l2_packet_send(wpa_s->l2_wai,
...
static int wpa_supplicant_get_state_helper(void *ctx)
{
	...
	/*
	* wpa_printf(MSG_INFO, "wpa_supplicant_get_state_helper,
	* ctx = %p, wpa_s = %p , sizeof(int) = %d, sizof(wpa_states) = %d", ctx,
...
void wpa_supplicant_deauthenticate_helper(void *ctx, int reason_code)
{
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)ctx;
...
void wpa_supplicant_set_state_helper(void *ctx, int state)
{
	...
	/*
	* wpa_printf(MSG_INFO, "wpa_supplicant_set_state_helper,
	* ctx = %p, wpa_s = %p , sizeof(int) = %d, sizof(wpa_states) = %d", ctx,
	* wpa_s, sizeof(int), sizeof(wpa_states));
	*/
...
static int wpa_supplicant_msg_send_helper(void *priv,
			const u8 *msg_in,
