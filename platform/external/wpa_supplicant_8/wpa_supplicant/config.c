				...
				const char *value)
{
	...
	if (*value == '"') {
		...
		if ((ssid->proto & WPA_PROTO_WAPI) != 0) {
			if (len >= 127)
				wpa_printf(MSG_INFO,
					"WAPI cannot accept any length >= 127 psk\n");
			else
				wpa_printf(MSG_INFO,
					"Accept the length = %lu psk\n", len);
	...
	if ((ssid->proto & WPA_PROTO_WAPI) != 0) {
		...
		if (ssid->len_wapi_hex_psk > 64) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid WAPI Hex '%s'. "
				"lenght too long",line, value);
			return -1;
		}
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if ((ssid->proto & WPA_PROTO_WAPI)!=0 && ssid->psk_set) {
		if (ssid->len_wapi_hex_psk <= 64) {
			...
			wpa_printf(MSG_ERROR,
				"[WAPI-Debug]ssid->len_wapi_hex_psk is too long\n");
			return NULL;
		}
	} else {
		if (ssid->psk_set)
				     ...
				     struct wpa_ssid *ssid)
{
	...
	if (ssid->proto & WPA_PROTO_WAPI) {
		wpa_printf(MSG_DEBUG,"[Debug] %s =>  proto: 0x%x",
			__FUNCTION__, ssid->proto & WPA_PROTO_WAPI);
		   ...
		   int line)
{
	...
	if (ssid == NULL || var == NULL || value == NULL)
		return -1;
#ifdef CONFIG_MTK_PRESET_SSID
	if (ssid->read_only && os_strncmp(var, "priority", 8) &&
		os_strncmp(var, "disabled", 8)) {
