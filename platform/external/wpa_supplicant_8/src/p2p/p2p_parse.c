...
static int
wfd_parse_attribute(u8 id, const u8 *data, u16 len,
			       struct p2p_message *msg)
{
	const u8 *pos;
	size_t i, nlen;
	if (len <= 0) {
		wpa_printf(MSG_DEBUG, "WFD msg len invalid");
		return 0;
	}
	switch (id) {
	...
	case WFD_SUBELEMENT_ID_COUPLED_SINK_INFO:
		...
		*/
		if (len == WFD_ATTRI_MAX_LEN_COUPLED_SINK_INFO) {
			...
			* kalMemCopy(prWfdInfo->aucCoupledSinkAddress,
			* prWfdAttriCoupldSinkStatus->aucCoupledSinkAddress, MAC_ADDR_LEN);
			*/
...
int p2p_parse_wfd_ie(const struct wpabuf *buf, struct p2p_message *msg)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Parsing WFD IE : buflen=%lu", wpabuf_len(buf));
...
#ifdef CONFIG_MTK_P2P
static int
wfd_session_info_text(const u8 *si, size_t si_len,
			char *buf, char *end)
{
	char *pos = buf;
	int ret;
...
int
wfd_session_info_parse(const u8 *si, size_t si_len,
			 struct wfd_session_info *info)
{
	const u8 *s, *send;
	...
	while (s < send) {
		...
		if (pCliDesc->ucLength != WFD_ATTRI_MAX_LEN_WFD_SESSION_INFO) {
			wpa_printf(MSG_DEBUG, "WFD: Wrong client descriptor length (%u) "
				"in session information", s[0]);
			return -1;
		}
		...
		*/
		...
		/*
		* os_memcpy(cli->wfd_sink_mac,
		* pCliDesc->aucCoupledSinkAddress,ETH_ALEN);
		*
		* wpa_printf(MSG_DEBUG, "WFD: dev " MACSTR " info 0x%x tp %u Assoc "
		*		MACSTR  " in session information",
		*		MAC2STR(cli->wfd_device_addr),
		*		cli->wfd_dev_info,
		*		cli->wfd_max_throughput,
		*		MAC2STR(cli->wfd_associated_bssid));
...
int wfd_attr_text(struct wpabuf *data, char *buf, char *end)
{
	...
	if ((msg.wfd_session_info_len>0) && msg.wfd_session_info) {
		...
		ret = wfd_session_info_text(msg.wfd_session_info,
					msg.wfd_session_info_len,
					pos, end);
		if (ret < 0)
			goto _FINISH;
		pos += ret;
...
int
wfd_attr_text_append(struct wpabuf *data, char *buf,
			char *end, unsigned int flags)
{
	struct p2p_message msg;
	char *pos = buf;
	int ret;
	...
	if ((msg.wfd_session_info_len>0) && msg.wfd_session_info) {
		ret = wfd_session_info_text(msg.wfd_session_info,
				msg.wfd_session_info_len, pos, end);
		if (ret < 0)
			goto _FINISH;
		pos += ret;
	}
